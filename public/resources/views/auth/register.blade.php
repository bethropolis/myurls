...
@section('content')
    <div class="max-w-md mx-auto my-8 bg-white p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-bold mb-4 text-pink-500 ">Sign Up</h2>

        <form action="{{ route('signup') }}" method="POST">
            @csrf

            <div class="mb-4">
                <label for="name" class="block mb-1 font-mono font-bold">Name</label>
            ...
            <div class="mb-4">
                <label for="password_confirmation" class="block mb-1 font-mono font-bold">Confirm Password</label>
                <input type="password" name="password_confirmation" id="password_confirmation" class="w-full px-4 py-2 border rounded-lg">
            </div>

            <div class="mt-6">
                <button type="submit" class="px-4 py-2  bg-pink-500 hover:bg-purple-500 duration-300 text-white rounded-lg">Sign Up</button>
            </div>
        </form>
    </div>
@endsection
